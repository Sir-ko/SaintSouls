...
SubShader {
    ...
    Pass {
        ...
        fixed4 PixShader(pixel_t input) : SV_Target
        {
            ...
            faceColor *= tex2D(_FaceTex, input.textures.xy + float2(_FaceUV_nowSpeedX, _FaceUV_nowSpeedY) * _Time.y);
            outlineColor *= tex2D(_OutlineTex, input.textures.zw + float2(_OutlineUV_nowSpeedX, _OutlineUV_nowSpeedY) * _Time.y);
            ...
            float3 bump = UnpackNormal(tex2D(_BumpMap, input.textures.xy + float2(_FaceUV_nowSpeedX, _FaceUV_nowSpeedY) * _Time.y)).xyz;
